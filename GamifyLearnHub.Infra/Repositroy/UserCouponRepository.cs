...
namespace GamifyLearnHub.Infra.Repositroy
{
    public class UserCouponRepository : IUserCouponRepository
    {
        ...
        public async Task<List<Usercoupon>> GetAllUserCoupons(int userId)
        {
            ...
            var result = await _dbContext.Connection.QueryAsync<Usercoupon, Coupon, Usercoupon>
                ("UserCoupon_Package.GetAllUserCoupons", (userCoupon, coupon) =>
                {
                    userCoupon.Coupon = coupon;
                    return userCoupon;
                }, p,splitOn:"couponId", commandType: CommandType.StoredProcedure);
            return result.ToList();
        }
        ...
        {
            var p = new DynamicParameters();
            p.Add("coupon_name", couponName, dbType: DbType.String, direction: ParameterDirection.Input);
            p.Add("user_id", userId, dbType: DbType.Int32, direction: ParameterDirection.Input);
            var result = await _dbContext.Connection.QueryAsync<Usercoupon, Coupon, Usercoupon>
                ("UserCoupon_Package.GetCouponByNameAndUserId"
                , (userCoupon, coupon) =>
                {
                    userCoupon.Coupon = coupon;
                    return userCoupon;
        ...
        {
            var p = new DynamicParameters();
            p.Add("userCoupon_id", userCouponId, dbType: DbType.Int32, direction: ParameterDirection.Input);
            p.Add("rows_effected", dbType: DbType.Int32, direction: ParameterDirection.Output);
            await _dbContext.Connection.ExecuteAsync
                ("UserCoupon_Package.UpdateUserCouponStatus", p, commandType: CommandType.StoredProcedure);
